...
(() => {
  ...
  function createBGM() {
    ...
    function applyModeParams() {
      if (!A.ctx) return;
      const targetCut = A.bossMode ? 2400 : 1100;
      const targetQ   = A.bossMode ? 1.15 : 0.7;
    ...
    function scheduleLoop4Bars(t0, beat, urgent) {
      ...
      const motif = [
        [69,72,76,74,76,72,69,69],
        [69,72,74,72,76,74,72,69],
        [71,74,78,76,78,74,71,71],
        [69,72,76,74,72,69,69,69],
      ];
    ...
    function schedule() {
      ...
      if (!A.isOn) { A._timer = setTimeout(schedule, 120); return; }
  ...
  window.TEL = {
    start(stage) {
      ...
      const BGM = createBGM();
      const syncBgmButton = () => { if (btnBgm) btnBgm.textContent = BGM.isOn ? "BGM: ON" : "BGM: OFF"; };
      ...
      const boss = {
        ...
        r: 40,
        t: 0,
        hp: stage.boss?.maxHP ?? 5000,
        maxHP: stage.boss?.maxHP ?? 5000,
        shootCD: 1.0,
        phase: 0
      };
      ...
      function resetGame() {
        ...
        boss.y = -220;
        boss.t = 0;
        boss.maxHP = stage.boss?.maxHP ?? 5000;
        boss.hp = boss.maxHP;
        boss.shootCD = 1.0;
        boss.phase = 0;
      ...
      function firePlayer(dt) {
        ...
        const n = lv;
        const maxAngle = isLuca ? (0.10 + 0.06 * (lv - 1)) : (0.02 + 0.005 * (lv - 1));
        for (let i=0;i<n;i++){
      ...
      function shootEnemyDown(e) {
        eBullets.push({ x:e.x, y:e.y+18, vx:0, vy:rand(240,320), r:4.2, dmg:12, life:3.0, t:0 });
      }
      ...
      function updateBossShooting(dt) {
        ...
        if (boss.phase === 0) { shootBossFan(5, 300, 0.55); boss.shootCD = 1.05; }
        else if (boss.phase === 1) { shootBossFan(7, 320, 0.75); shootBossAimedBurst(3, 340, 0.10); boss.shootCD = 0.95; }
        else { shootBossFan(9, 340, 0.95); shootBossAimedBurst(5, 360, 0.12); boss.shootCD = 0.90; }
      }

      // items
      function spawnItem(type, x, y) { items.push({ type, x, y, vy: 140, r: 12 }); }
      function maybeDropItemsOnKill(x, y) {
        const rates = stage.dropRates || { gun:0.20, heal:0.10 };
        const r = Math.random();
        if (r < (rates.heal ?? 0.10)) { spawnItem("HEAL", x, y); return; }
        if (r < (rates.heal ?? 0.10) + (rates.gun ?? 0.20)) spawnItem("GUN", x, y);
      }
      ...
      function drawEnemy(e) {
        const img = sprites.robo_blue, size = stage.enemySize ?? 64;
        if (imgReady(img)) drawSpriteCentered(ctx, img, e.x, e.y, size);
        else { ctx.fillStyle="rgba(80,160,255,0.95)"; ctx.beginPath(); ctx.arc(e.x,e.y,e.r+6,0,Math.PI*2); ctx.fill(); }
      }

      function drawBoss() {
        if (!boss.active) return;
        const img = sprites.robo_red, size = stage.boss?.size ?? 210;
        if (imgReady(img)) drawSpriteCentered(ctx, img, boss.x, boss.y, size);
        else { ctx.fillStyle="rgba(255,90,90,0.95)"; ctx.beginPath(); ctx.arc(boss.x,boss.y,boss.r,0,Math.PI*2); ctx.fill(); }
      }

      function drawPlayer() {
        const img = (player.char==="LUCA") ? sprites.luca : sprites.marca;
        const size = stage.playerSize ?? 176;
        if (imgReady(img)) drawSpriteCentered(ctx, img, player.x, player.y, size);
        else { ctx.fillStyle="rgba(232,242,255,0.9)"; ctx.beginPath(); ctx.arc(player.x,player.y,22,0,Math.PI*2); ctx.fill(); }
      }

      function drawItems() {
        for (const it of items) {
          const img = (it.type==="GUN") ? sprites.gun : sprites.fish_can;
          if (imgReady(img)) drawSpriteCentered(ctx, img, it.x, it.y, 44);
          else { ctx.fillStyle = it.type==="GUN" ? "rgba(255,220,120,0.95)" : "rgba(118,210,200,0.95)"; ctx.beginPath(); ctx.arc(it.x,it.y,10,0,Math.PI*2); ctx.fill(); }
        }
      }
      ...
      function drawHUD() {
        ...
        if (boss.active) {
          const w=260,h=12,x=(LOG_W-w)/2,y=40;
          ctx.fillStyle="rgba(0,0,0,0.35)";
      ...
      function loop(t) {
        ...
        if (stateNow === State.PLAY) {
          ...
          if (!boss.active) {
            ...
            if (spawnCD <= 0) {
              spawnEnemyFromStage();
              const [a,b] = stage.spawnInterval ?? [0.42, 0.74];
              spawnCD = rand(a,b);
            }
          }
